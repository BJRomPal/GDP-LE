...
print("""The countries represented in the data are:
* Chile
* China
* Germany
* Mexico
...
print('The data represents the years 2000 to 2015')
...
plt.title('Comparative bar chart of countries GDP')
plt.xlabel('Countries')
plt.ylabel('GDP in Trillions of U.S. Dollars')
plt.savefig('GPD_Comparisson.png')
plt.show()
...
plt.title('Comparative bar chart of countries Life Expectancy')
plt.xlabel('Countries')
plt.ylabel('Life Expectancy in years')

plt.savefig('Life_Expectancy_Comparisson.png')
...
chil_zim = df[(df.Country == 'Chile') | (df.Country == 'Zimbabwe')].reset_index()

plt.bar(chil_zim['Country'], chil_zim['GDP'], color = 'green')
plt.title("Comparative bar chart of Chile's and Zimbabwe's GDP")
plt.xlabel('Countries')
plt.ylabel('GDP in Trillions of U.S. Dollars')

plt.savefig('GDP_Chile_Zimbabwe.png')
...
print("""In the case of Zimbabwe it appears that the size of the GDP affects it's Life Expectancy. 
However it is not the case with the rest of the other countris
""")
...
ax = sns.barplot(x='Country', y='GDP', data=chil_zim, hue='Year', palette='cubehelix')

ax.set_title('GDP for Chile and Zimbabwe over the years')
ax.set_xlabel('Countries')
ax.set_ylabel('GDP in Trillions of U.S. Dollars')

plt.savefig('GDP_Years_Chile_Zimbabwe.png')
...
sns.set_theme(font='Verdana', palette='Pastel1', style='whitegrid', font_scale=1)

ax = sns.violinplot(df['Country'], df['LEABY'])
ax.set_title('Report on Life Expectancy per Country')
ax.set_xlabel('Coutries')
ax.set_ylabel('Life Expectancy in years')

plt.savefig('Violin_Life_Expectancy.png')
...
print('The distribution in all the countries seem to be normal except in Zimbabwe where life expectancy has chaged a lot')
...
ax = sns.barplot(x='Country', y='GDP', data=df, hue='Year', palette='rocket_r')

ax.set_title('GDP per Country over the years')
ax.set_xlabel('Countries')
ax.set_ylabel('GDP in Trillions of U.S. Dollars')
...
ax = sns.barplot(x='Country', y='LEABY', data=df, hue='Year', palette='ch:start=.2,rot=-.3')

ax.set_title('Life Expectancy per Country over the years')
ax.set_xlabel('Countries')
ax.set_ylabel('Life expectancy at birth in years')
#ax.set_yticks([40, 50, 60, 70, 80])
ax.set(ylim=(40, 90))

plt.savefig('Life_Expectancy_Over_Years.png')
...
print("""The countries' bars that changed the most are those of Zimbabwe and China.
The years that the data had a big change where for Zimbabwe from 2010 to 2015 and for China from 2000 to 2005.
Zimbabwe experienced the least change in GDP over time.
All the examined countries experienced an increase in both their GDP, however China and the USA had the biggest increase in it.
Chile and Germany are the only two countries that surpassed the age of 80 in Life Expectancy.
Mexico's Life Expectancy and GDP seem to correlate one to the other experiencing the same changes over the years. 
Zimbabwe's improve in Life Expectancy had a direct correlation with the increase of the country GDP. 
An improve in the GDP means that the country can expend more money in health care and other services that improve Life Expectancy.
In the case of China the data shows the changes that the country has been making in their economý. 
"""
)
...
g = sns.FacetGrid(data=df, col='Year', hue='Country', col_wrap=4, height=2, palette='Set1')
g = (g.map(sns.scatterplot, 'GDP', 'LEABY', edgecolor="red").add_legend())
g.set_axis_labels('GDP', 'Life Expectancy')
...
print("""USA is the country that has moved the most on the X axis.
Zimbabwe is the country that has moved the mosto on the y axis.
No. It isn't. It's consistent with what we have seen in previous charts. 
However the changes in Zimbabwe's growth in Life Expectancy seems to have other source than only GDP growth.
)
...
g3.set(xticks=[2000, 2005, 2010, 2015])
g3 = (g3.map(plt.plot, "Year", "LEABY"))
g3.set_axis_labels('Years', 'Life Expectancy')
...
g4.set(xticks=[2000, 2005, 2010, 2015])
g4 = (g4.map(plt.plot, "Year", "GDP", color='green'))
...
print('USA has the highest GDP and Zimbabwe has the lowest.')
...
print('Germany has the highest life expectancy and Zimbabwe has the lowest.')
...
print('What happened in Zimbabwe in the past 10 years that increased the Life Expectancy so drastically?')
print('\n')
print("""Zimbabwe's Life Expectancy is actually recoverin from some major drastic fall that took place since 1987.
The two main causes that reduced the Life Expectancy was the deterioration of the Economy and the spread of the HIV disease. 
Since then the government started a program to reduce the percentage of infection from HIV specially among women.
...
print('The history and cultural values have a relationship with the GDP and life expectancy.')
print('It would have helped to the poryect to know the economic and health policies that each country had. The dataset should include those policies.')
print('Specially in the case of Zimbabwe the GDP had less effect to growth of the Life Expectancy. In fact the growth can be attributable to better health policies.')
print('I believe that the Scatterplot illustrated better the different relationships')
